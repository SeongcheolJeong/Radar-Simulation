# Graph Lab Document Map

## Purpose

Use this page when you know you need Graph Lab documentation, but you do not know which document to open first.

This page is the routing map for the Graph Lab documentation set.

## Fast Routing

| If you want to... | Open this first | Then open |
| --- | --- | --- |
| launch Graph Lab as fast as possible | [Frontend Runtime Purpose Presets](280_frontend_runtime_purpose_presets.md) | [Graph Lab Live Checklist](306_graph_lab_live_checklist.md) |
| understand the full screen and workflow | [Graph Lab UX Manual](300_graph_lab_ux_manual.md) | [Graph Lab Button Scenario Guide](302_graph_lab_button_scenario_guide.md) |
| follow the shortest click path while Graph Lab is already open | [Graph Lab Live Checklist](306_graph_lab_live_checklist.md) | [Artifact Inspector Quick Guide](304_artifact_inspector_quick_guide.md) |
| know which button to press for one task | [Graph Lab Button Scenario Guide](302_graph_lab_button_scenario_guide.md) | [Graph Lab Live Checklist](306_graph_lab_live_checklist.md) |
| read the right panel without the full manual | [Artifact Inspector Quick Guide](304_artifact_inspector_quick_guide.md) | [Graph Lab Live Checklist](306_graph_lab_live_checklist.md) |
| stay in Korean | [Graph Lab UX 사용 매뉴얼](301_graph_lab_ux_manual_ko.md) | [Graph Lab 실사용 체크리스트](307_graph_lab_live_checklist_ko.md) |

## Recommended Reading Order

### First-time operator

1. [Frontend Runtime Purpose Presets](280_frontend_runtime_purpose_presets.md)
2. [Graph Lab UX Manual](300_graph_lab_ux_manual.md)
3. [Graph Lab Live Checklist](306_graph_lab_live_checklist.md)

### Operator who is already in the UI

1. [Graph Lab Live Checklist](306_graph_lab_live_checklist.md)
2. [Graph Lab Button Scenario Guide](302_graph_lab_button_scenario_guide.md)
3. [Artifact Inspector Quick Guide](304_artifact_inspector_quick_guide.md)

### Operator who only needs the evidence panel

1. [Artifact Inspector Quick Guide](304_artifact_inspector_quick_guide.md)
2. [Graph Lab Live Checklist](306_graph_lab_live_checklist.md)

## What Each Document Is For

- [Frontend Runtime Purpose Presets](280_frontend_runtime_purpose_presets.md)
  - runtime tracks, presets, compare workflow, provider controls
- [Graph Lab UX Manual](300_graph_lab_ux_manual.md)
  - screen map, panel roles, run sequences
- [Graph Lab Live Checklist](306_graph_lab_live_checklist.md)
  - shortest practical click path in the live UI
- [Graph Lab Button Scenario Guide](302_graph_lab_button_scenario_guide.md)
  - task-to-button routing
- [Artifact Inspector Quick Guide](304_artifact_inspector_quick_guide.md)
  - right-panel reading order and artifact evidence interpretation

## Korean Set

- [Graph Lab UX 사용 매뉴얼](301_graph_lab_ux_manual_ko.md)
- [Graph Lab 실사용 체크리스트](307_graph_lab_live_checklist_ko.md)
- [Graph Lab 버튼 시나리오 가이드](303_graph_lab_button_scenario_guide_ko.md)
- [Artifact Inspector 빠른 읽기 가이드](305_artifact_inspector_quick_guide_ko.md)

## Related Documents

- [Documentation Index](README.md)
- [Generated Reports Index](reports/README.md)
- [Classic Dashboard Document Map](320_classic_dashboard_doc_map.md)
