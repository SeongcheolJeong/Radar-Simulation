...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run the canonical Sionna-style RT scene-backend parity case and emit a stable report."
    )
    p.add_argument(
        "--output-root",
        default="data/runtime_golden_path/scene_backend_parity_sionna_rt_latest",
        help="Directory for persistent parity artifacts.",
    )
    p.add_argument(
        "--output-json",
        default="docs/reports/scene_backend_parity_sionna_rt_latest.json",
...
def main() -> None:
    ...
    print("run_scene_backend_parity_sionna_rt: done")
