...
PYTHONPATH="$ROOT_DIR/src" python3 "$ROOT_DIR/scripts/run_scene_runtime_po_sbr_pilot.py" \
  --output-root "$OUTPUT_ROOT" \
  --output-summary-json "$SUMMARY_JSON" \
  --po-sbr-repo-root "$PO_SBR_REPO_ROOT" \
  --geometry-path "$GEOMETRY_PATH"

PYTHONPATH="$ROOT_DIR/src" python3 "$ROOT_DIR/scripts/validate_scene_runtime_po_sbr_executed_report.py" \
